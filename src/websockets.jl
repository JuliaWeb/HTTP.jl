...
# ─── WsHandler ───
# Bridges the Reseau pipeline with the AwsHTTP WebSocket codec.
# Installed into the H1Connection's pipeline after HTTP 101 upgrade.

mutable struct WsHandler
    pipeline::Any   # PipelineState
    aws_ws::Any     # AwsHTTP.WebSocket
    wslock::ReentrantLock
    ws::Any
end

WsHandler(pipeline, aws_ws, ws) = WsHandler(pipeline, aws_ws, ReentrantLock(), ws)

# Collect outgoing frames from the AwsHTTP WebSocket codec and send them
# through the pipeline. Caller MUST hold handler.wslock.
function _ws_channel_flush!(handler::WsHandler)
...
function _shutdown_ws_channel!(handler::WsHandler)
    pipeline = handler.pipeline
    pipeline === nothing && return
...
# Create an AwsHTTP WebSocket and WsHandler, then install
# the read handler into the H1Connection's pipeline.
function _create_ws_handler!(ws::WebSocket, pipeline, is_client::Bool)
    ...
    ws = with_redirect("GET", uri, headers, nothing, redirect, redirect_limit, redirect_method, forwardheaders) do method, uri, headers, body
        ...
        with_request(reqclient, method, path, headers, body, nothing, false, (username !== nothing && password !== nothing) ? "$username:$password" : userinfo(uri), bearer, modifier, false, cookies, cookiejar, verbose) do req
            ...
            with_connection(reqclient) do conn
                ...
                # Swap the H1Connection handler with our WsHandler
                h1conn = stream.aws_stream.owning_connection
                _create_ws_handler!(ws, h1conn.pipeline, true)
                if !isempty(buffered)
                    handler = ws.handler
                    handler !== nothing || throw(WebSocketError(CloseFrameBody(1006, "WebSocket not connected")))
                    @lock handler.wslock begin
...
function websocket_upgrade_function(f; suppress_close_error::Bool=false, maxframesize::Integer=typemax(Int), maxfragmentation::Integer=DEFAULT_MAX_FRAG, done=nothing)
    function websocket_upgrade(stream::Stream)
        ...
        ws.handshake_request = req
        ws.handshake_response = resp
        # Get the H1Connection's pipeline and swap in our WebSocket handler
        h1conn = stream.aws_stream.owning_connection
        _create_ws_handler!(ws, h1conn.pipeline, false)
