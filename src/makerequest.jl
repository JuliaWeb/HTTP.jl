...
function request(method, url, h=Header[], b::RequestBodyTypes=nothing;
    ...
    return with_redirect(allocator, method, uri, headers, body, redirect, redirect_limit, redirect_method, forwardheaders) do method, uri, headers, body
        reqclient = @something(client, getclient(ClientSettings(scheme(uri), host(uri), getport(uri); kw...)))::Client
        with_retry_token(reqclient) do
            resp = with_connection(reqclient) do conn
                ...
                path = resource(uri)
                with_request(reqclient, method, path, headers, body, chunkedbody, decompress, (username !== nothing && password !== nothing) ? "$username:$password" : userinfo(uri), modifier, http2, cookies, cookiejar, verbose) do req
