...
`Sockets.TCPSocket` or `SSLContext` that it wraps.
...
A `Sockets.TCPSocket` or `SSLContext` connection to a HTTP `host` and `port`.
...
- `io::T`, the `Sockets.TCPSocket` or `SSLContext.
...
function getconnection(::Type{Sockets.TCPSocket},
                       host::AbstractString,
                       port::AbstractString;
                       keepalive::Bool=false,
                       kw...)::Sockets.TCPSocket
...
function getconnection(::Type{SSLContext},
    ...
    io = SSLContext()
    setup!(io, sslconfig)
    associate!(io, getconnection(Sockets.TCPSocket, host, port; kw...))
    hostname!(io, host)
    handshake!(io)
    return io
end
