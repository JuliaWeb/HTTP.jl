# Shared high-level workload used by package precompilation and runtime smoke tests.

function _precompile_wait_port(server; timeout_s::Float64=5.0)::Int
...
function _precompile_wait_ws_url(server::WebSockets.Server; timeout_s::Float64=5.0)::String
    deadline = time() + timeout_s
    while time() < deadline
        try
            return "ws://" * WebSockets.server_addr(server) * "/echo"
        catch
        end
        sleep(0.01)
    end
...
function _run_precompile_workload!()::Nothing
    ...
    try
        ...
        ws_url = _precompile_wait_ws_url(ws_server::WebSockets.Server)
