...
function shutdown(fn::Function)
    try
        ...
        @error "shutdown function $fn failed. $err"
    end
end
    ...
    while isopen(listener)
        try
            ...
            if e isa Base.IOError && e.code == Base.UV_ECONNABORTED
                ...
                err = current_exceptions_to_string()
                @errorv 2 "Server on $(listener.hostname):$(listener.hostport) errored. $err"
    ...
    try
        ...
        while isopen(c) && !closedorclosing(c.state) && isopen(listener)
            ...
            try
                ...
                err = current_exceptions_to_string()
                @debugv 1 "handle_connection startread error. $err"
                break
            end
            ...
            try
                ...
                err = current_exceptions_to_string()
                @logmsgv 1 level "handle_connection handler error. $err"
