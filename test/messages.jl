...
@testitem "Read methods" setup=[HTTPBin] begin
    isok(r) = r.status == 200
...
@testitem "Body - Response Stream" setup=[HTTPBin] begin
    using JSON
    isok(r) = r.status == 200
...
@testitem "Body - JSON Parse" setup=[HTTPBin] begin
    using JSON
    isok(r) = r.status == 200
    @testset for method in ("POST", "PUT", "DELETE", "PATCH")
        uri = "https://$httpbin/$(lowercase(method))"
...
@testitem "Write to file" setup=[HTTPBin] begin
    using JSON
