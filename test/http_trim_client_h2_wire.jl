...
Base.Experimental.entrypoint(_http_trim_h2_wire_server_entry, ())
...
function run_http_trim_client_h2_wire()::Nothing
    ...
    try
        ...
        server_task = errormonitor(Task(_http_trim_h2_wire_server_entry))
        ...
        try
            conn === nothing || close(conn::Reseau.TCP.Conn)
        catch
        end
        try
